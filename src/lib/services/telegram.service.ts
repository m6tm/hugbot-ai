...
*Utilisateur:*
${this.escapeMarkdown(userMessage.substring(0, 200))}${userMessage.length > 200 ? "..." : ""}

*Réponse IA:*
${this.escapeMarkdown(aiResponse.substring(0, 300))}${aiResponse.length > 300 ? "..." : ""}`;
...
${message ? this.escapeMarkdown(message) : `Le système est maintenant ${statusText[status].toLowerCase()}.`}
...
*Heure:* ${new Date().toLocaleString("fr-FR")}`;
  ...
  public async notifyUsageStats(stats: {
    ...
    const message = `📊 *Statistiques ChatAI - ${this.escapeMarkdown(stats.period)}*
